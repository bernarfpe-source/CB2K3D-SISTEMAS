    ...
    <Modal title={product ? "Editar Produto" : "Novo Produto"} onClose={onClose} width={900}>
      <div style={{ maxHeight: "70vh", overflowY: "auto", paddingRight: 8 }}>
        ...
        <div style={{ marginBottom: 32 }}>
          <h3 style={{ fontSize: 14, fontWeight: 700, color: "#1C1C1E", borderBottom: "1px solid #E5E5EA", paddingBottom: 8, marginBottom: 16 }}>PERFIL DE IMPRESSÃO (SETUP)</h3>

          {/* G-Code Import */}
          <div style={{ background: "#E8F5E9", border: "1px dashed #34C759", borderRadius: 8, padding: 12, marginBottom: 20, display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ background: "#34C759", color: "#fff", width: 40, height: 40, borderRadius: 20, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>📂</div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 13, fontWeight: 700, color: "#1C1C1E" }}>Importar Dados do G-Code</div>
              <div style={{ fontSize: 11, color: "#666" }}>Arraste o arquivo .gcode ou .3mf fatiado aqui para preencher tudo automaticamente.</div>
            </div>
            <input type="file" accept=".gcode,.gco,.m3f,.3mf" id="gcode-upload" style={{ display: "none" }} onChange={(e) => {
              const file = e.target.files[0];
              if (!file) return;
              ...
              Reader.onload = (event) => {
                ...
                // PARSER LOGIC: Extract metadata from G-code comments
                // 1. Time (Bambu: ; total estimated time_seconds = 123)
                let timeMatch = text.match(/; total estimated time_seconds\s*=\s*(\d+)/i) || text.match(/; estimated structural printing time \(normal mode\)\s*=\s*(\d+)/i);
                let totalMin = 0;
                if (timeMatch) {
                  totalMin = Math.round(parseInt(timeMatch[1]) / 60);
                } else {
                  // Cura (;TIME:12345)
                  timeMatch = text.match(/;TIME:(\d+)/);
                  if (timeMatch) totalMin = Math.round(parseInt(timeMatch[1]) / 60);
                }

                // 2. Weight ( ; filament_used_g = 12.3)
                let weightMatch = text.match(/; filament_used_g\s*=\s*([\d\.]+)/i);
                let totalWeight = 0;
                if (weightMatch) totalWeight = parseFloat(weightMatch[1]);

                // 3. Settings
                const layerMatch = text.match(/; layer_height\s*=\s*([\d\.]+)/i);
                const wallsMatch = text.match(/; wall_loops\s*=\s*(\d+)/i);
                const infillMatch = text.match(/; sparse_infill_density\s*=\s*(\d+%?)/i);
                const nozzleMatch = text.match(/; nozzle_diameter\s*=\s*([\d\.]+)/i);

                setForm(prev => {
                  const newPerfil = { ...prev.perfil };
                  if (layerMatch) newPerfil.camada = layerMatch[1];
                  if (wallsMatch) newPerfil.paredes = parseInt(wallsMatch[1]);
                  if (infillMatch) newPerfil.preenchimento = infillMatch[1];
                  if (nozzleMatch) newPerfil.bico = nozzleMatch[1];

                  let newPartes = [...(prev.partes || [])];
                  // If single part or empty, update simple weight/time
                  if (newPartes.length <= 1 && totalWeight > 0) {
                    newPartes = [{
                      ...(newPartes[0] || { id: Date.now(), nome: "Peça Principal", materialId: "", foto: "" }),
                      peso: totalWeight,
                      tempo: totalMin || prev.tempoImpressao
                    }];
                  }

                  // Sync Composicao
                  const newComposicao = newPartes.map(p => {
                    const m = materials.find(x => String(x.id) === String(p.materialId));
                    return { materialId: p.materialId, peso: parseFloat(p.peso) || 0, tipo: m?.tipo || "PLA", cor: m?.cor || "" };
                  });

                  showToast(`Dados Extraídos! Tempo: ${totalMin}m, Peso: ${totalWeight}g`, "success");
                  ...
                  return {
                    ...
                    tempoImpressao: totalMin || prev.tempoImpressao
                  };
                });
              };
              // Read first 1MB is usually enough for headers? 
              // Bambu puts headers at END. So read full file text (dangerous if > 100MB).
              // Max file size check? G-code is usually < 50MB.
              if (file.size > 50 * 1024 * 1024) {
                alert("Arquivo muito grande. O parser pode travar. Tente um arquivo menor.");
              }
              Reader.readAsText(file);
            }} />
            <label htmlFor="gcode-upload" style={{ background: "#fff", border: "1px solid #34C759", color: "#34C759", padding: "6px 12px", borderRadius: 6, fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
              Escolher Arquivo
            </label>
          </div>
